...
******************************************************************************************/
float Beta_Adjust[5]={0.004,0.0075,0.01,0.004,0.0045};//{0.04,0.03,0.025,0.02,0.01};{0.05,0.03,0.025,0.02,0.01};
float BETADEF=0.01;
...
void AHRSUpdate_GraDes_TimeSync(float gx, float gy, float gz, float ax, float ay, float az)
{
  ...
  if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f)))
  {
    ...
    s2 *= recipNorm;
    s3 *= recipNorm;
    BETADEF=Beta_Adjust[1]+0.025*IMU_Dt*constrain(Gyro_Length_Filter,0,400);
  ...
  if((Mag_Yaw>90 && att.angle[_YAW]<-90)
    ...
    att.angle[_YAW] = -att.angle[_YAW] * 0.99f + Mag_Yaw * 0.01f;
  else att.angle[_YAW] = att.angle[_YAW] * 0.99f + Mag_Yaw * 0.01f;
