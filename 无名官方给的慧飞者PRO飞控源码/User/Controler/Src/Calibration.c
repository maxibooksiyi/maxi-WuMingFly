...
void Accel_Calibration_Check(void)
{
             ...
             &&Controler_State==Lock_Controler)//必须为上锁状态才可以进入校准模式
  {
    ...
    Cal_Flag=0;
    Bling_Set(&Light_1,1000,100,0.5,0,GPIOC,GPIO_Pin_6,1);
    Bling_Set(&Light_2,1000,100,0.5,0,GPIOC,GPIO_Pin_7,1);
    Bling_Set(&Light_3,1000,100,0.5,0,GPIOC,GPIO_Pin_8,1);
    flight_direction=6;
...
uint8_t Accel_Calibartion(void)
{
     ...
     &&Accel_Calibration_All_Finished==0)//6面全部校准完毕
  {
    ...
    if(Cal_Flag==TRUE)//加速度计校准成功
    {
      ...
      Bling_Set(&Light_1,3000,1000,0.3,0,GPIOC,GPIO_Pin_6,0);
      Bling_Set(&Light_2,3000,1000,0.5,0,GPIOC,GPIO_Pin_7,0);
      Bling_Set(&Light_3,3000,1000,0.8,0,GPIOC,GPIO_Pin_8,0);
    }
    else//加速度计校准失败
    {
      Bling_Mode=0;//恢复正常指示模式
      Bling_Set(&Light_1,5000,200,0.3,0,GPIOC,GPIO_Pin_6,0);
      Bling_Set(&Light_2,5000,200,0.5,0,GPIOC,GPIO_Pin_7,0);
      Bling_Set(&Light_3,5000,200,0.8,0,GPIOC,GPIO_Pin_8,0);
...
bool Parameter_Init(void)
{
  bool success=TRUE;
  /************加速度计零偏与标度值*******/
  Parameter_Read_Flag.accel_off=ReadFlashParameterThree(ACCEL_X_OFFSET,
                                                        &Accel_Offset_Read.x,
                                                        &Accel_Offset_Read.y,
                                                        &Accel_Offset_Read.z);
  
  Parameter_Read_Flag.accel_scale=ReadFlashParameterThree(ACCEL_X_SCALE,
                                                          &Accel_Scale_Read.x,
                                                          &Accel_Scale_Read.y,
                                                          &Accel_Scale_Read.z);
  /************磁力计零偏****************/
  Parameter_Read_Flag.mag=ReadFlashParameterThree(MAG_X_OFFSET,
                                                  &Mag_Offset_Read.x,
                                                  &Mag_Offset_Read.y,
                                                  &Mag_Offset_Read.z);
  // sanity check scale
  ...
  if(success==TRUE
     &&Parameter_Read_Flag.accel_off!=0x00
       &&Parameter_Read_Flag.accel_scale!=0x00)//Flash内数据正常，更新加速度校正值
  {
  ...
  /**********磁力计中心偏执获取************/
  if(Parameter_Read_Flag.mag!=0x00)
  {
...
void Mag_Calibration_Check(void)
{
    ...
    //进入磁力计校准模式
  {
    ...
    Bling_Set(&Light_1,1000,500,0.2,0,GPIOC,GPIO_Pin_6,1);
    Bling_Set(&Light_2,1000,500,0.5,0,GPIOC,GPIO_Pin_7,1);
    Bling_Set(&Light_3,1000,500,0.7,0,GPIOC,GPIO_Pin_8,1);
...
uint8_t Mag_Calibartion(Mag_Unit MagData,Vector3f_Body Circle_Angle_Calibartion)
{
       ...
       &&Mag_Is_Okay_Flag[2]==1)//三面数据全部采集完毕，计算磁力计零点
  {
    ...
    Bling_Set(&Light_1,5000,1000,0.3,0,GPIOC,GPIO_Pin_6,0);
    Bling_Set(&Light_2,5000,1000,0.5,0,GPIOC,GPIO_Pin_7,0);
    Bling_Set(&Light_3,5000,1000,0.8,0,GPIOC,GPIO_Pin_8,0);
...
void ESC_Calibration_Check(void)
{
    ...
    //进入ESC校准模式
  {
    ...
    WriteFlashParameter(ESC_CALIBRATION_FLAG,
                        ...
                        &Table_Parameter);
    Bling_Set(&Light_1,5000,500,0.2,0,GPIOC,GPIO_Pin_6,1);
    Bling_Set(&Light_2,5000,500,0.2,0,GPIOC,GPIO_Pin_7,1);
    Bling_Set(&Light_3,5000,500,0.2,0,GPIOC,GPIO_Pin_8,1);
    Page_Number=15;
  }
}
...
void Horizontal_Calibration_Check(void)
{
             ...
             &&Controler_State==Lock_Controler)//必须为上锁状态才可以进入校准模式
  {
    ...
    Bling_Set(&Light_1,500,100,0.2,0,GPIOC,GPIO_Pin_6,0);
    Bling_Set(&Light_2,500,100,0.2,0,GPIOC,GPIO_Pin_7,0);
    Bling_Set(&Light_3,500,100,0.2,0,GPIOC,GPIO_Pin_8,0);
  }
}
